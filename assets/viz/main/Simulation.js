...
class Simulation {
  ...
  setTarget(targetName) {
    ...
    this.mcmc.function = MCMC.targets[targetName].function;
    this.mcmc.gradLogDensity = MCMC.targets[targetName].gradLogDensity;
    this.mcmc.function = MCMC.targets[targetName].function;
    this.mcmc.xmin = MCMC.targets[targetName].xmin;
    this.mcmc.xmax = MCMC.targets[targetName].xmax;
    ...
    const nx = 480,
      ny = 256,
      nz = 7;
    this.computeContours(this.mcmc.function, xmin, xmax, ymin, ymax, nx, ny, nz);
  ...
  computeContours(f, xmin, xmax, ymin, ymax, nx, ny, nz) {
    // get contours
    ...
    const gradientArray = new Gradient()
      .setColorGradient("#D4B0C2", "#844867")
      .setMidpoint(100)
      .getColors();
    for (let j = 0; j < ny; ++j) {
      for (let i = 0; i < nx; ++i) {
        var base = 4 * ((ny - 1 - j) * nx + i);
        var value = Math.pow((data[i][j] - min) / (max - min), 1/2);
        const color = gradientArray[Math.round(value * (gradientArray.length - 1))];
        image.data[base] = color.r;
        image.data[base + 1] = color.g;
        image.data[base + 2] = color.b;
        image.data[base + 3] = 255//0 | Math.pow(value, 1/4) * 255;
      }
    }
